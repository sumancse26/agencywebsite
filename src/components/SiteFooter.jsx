...
import { GET_ALL_SOCIAL_LIST } from '@/apiCalling/agencyWebSite';
import Link from 'next/link';

const getSocialLink = async () => {
	try {
		const res = await GET_ALL_SOCIAL_LIST();
		return res.data;
	} catch (err) {
		throw new Error(err);
	}
...
const SiteFooter = async () => {
	const socialList = await getSocialLink();
	return (
		<section>
			...
			<div className="py-20 bg-gray-900 radius-for-skewed">
				<div className="container mx-auto px-4">
						...
						<div className="w-full lg:w-auto">
							<ul className="flex flex-wrap lg:space-x-5 items-center">
								...
								<li className="hidden md:block">
									<svg
										className="mx-4 w-4 h-4 text-gray-800"
					...
					<div className="mt-8 flex flex-wrap justify-between items-center">
						<p className="order-last text-sm text-gray-400">© 2024. All rights reserved.</p>
						<div className="mb-4 lg:mb-0 order-first lg:order-last">
							{socialList?.map((item) => (
								<a
									...
									{/* "`/image/filter-icon-${category.icon}.png`" */}
								</a>
							))}
						</div>
					</div>
