import { GET_ALL_PROJECT_LIST } from '@/apiCalling/agencyWebSite';
import { getConvertedDate } from '@/utilities/dateFormate';

const getAllProject = async () => {
	try {
		const res = await GET_ALL_PROJECT_LIST();
		return res.data;
	} catch (err) {
		throw new Error(err);
	}
				...
				<div className="container px-4 mx-auto">
					<div className="mb-16 flex flex-wrap justify-center md:justify-center items-center">
						<div className="text-center lg:text-center">
							...
							<h2 className="text-4xl lg:text-5xl font-bold font-heading">All Projects</h2>
						</div>
					</div>
					<div className="flex flex-wrap -mx-4 mb-4">
						{allProjectData?.map((item) => (
							<div className="relative mb-4 w-full md:w-1/2 lg:w-1/3 px-4" key={item.id}>
								<div className="relative h-80 mb-5 mx-auto rounded-lg">
									...
									<div className="absolute inset-0 p-6 flex flex-col items-start">
										<span className="text-gray-400">{getConvertedDate(item.created_at)}</span>
										<p className="mb-auto text-xl lg:text-2xl text-white font-bold">{item.title}</p>
										<a
